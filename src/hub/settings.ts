/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改 Notion 凭证配置
 */
...
/**
 * POST /settings/save — 保存修改后的 Notion 凭证配置
 */
export async function handleSettingsSave(
  ...
  if (notionToken) newConfig.notion_token = notionToken;
  // database_id 允许清空，始终更新
  newConfig.notion_database_id = notionDatabaseId;
...
function renderConfigForm(
  ...
  const maskedToken = maskValue(cfg.notion_token || "");
  const maskedDbId = maskValue(cfg.notion_database_id || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Notion — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Notion 设置</h1>
    <p class="desc">修改您的 Notion 集成凭证配置</p>
    ${successMsg ? `<div class="success">${successMsg}</div>` : ""}

    <div class="current-config">
      <dt>当前 Integration Token</dt>
      <dd>${maskedToken || "（未配置）"}</dd>
      <dt>当前 Database ID</dt>
      <dd>${maskedDbId || "（未配置）"}</dd>
    </div>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="notion_token" type="password" placeholder="留空则不修改" />
      <p class="hint">以 ntn_ 开头的集成令牌</p>

      <label>Database ID</label>
      <input name="notion_database_id" placeholder="留空则清除" value="${escapeHtml(cfg.notion_database_id || "")}" />
      <p class="hint">默认写入的数据库 ID</p>
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-notion">自行部署</a>本 App</li>
        </ul>
      </div>
