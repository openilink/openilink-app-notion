 ...
 * - POST /api/v1/send — 记录 NotionToWx 通过 fetch 发送的消息
 * - POST /api/bot/message — 记录 HubClient.sendMessage 发送的消息
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && url.pathname === "/mock/event") {
      try {
        ...
        // 构造 HubEvent（Notion 项目 hub/types.ts 格式）
        const hubEvent = {
          v: "1",
          type: "event",
        ...
        // 计算签名（HMAC-SHA256 hex，与 Notion 项目的 verifySignature 一致）
